...
class SAM2Train_Lesion(SAM2Train):
    def __init__(self, fixed_annotation=None, num_ortho_slices=0, **kwargs):
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        for i in range(self.num_ortho_slices):
            init_cond_frames.append(start_frame_idx + 1 + i)
        ...
        if self.fixed_annotation is None:
            prompt_type = np.random.choice(['mask', 'line', 'arrow', 'point', 'box'])
        else:
            prompt_type = self.fixed_annotation

        initial_gt_mask = gt_masks_per_frame[0]
        if prompt_type == 'mask':
            backbone_out["mask_inputs_per_frame"][0] = initial_gt_mask
        else:
            if prompt_type == 'line':
                points, labels = find_furthest_points_brute(initial_gt_mask)
            elif prompt_type == 'arrow':
                pass
            elif prompt_type == 'point':
                points, labels = get_next_point(
                    gt_masks=initial_gt_mask,
                    pred_masks=None,
                    method=(
                ...
                points, labels = sample_box_points(initial_gt_mask)

            point_inputs = {"point_coords": points, "point_labels": labels}
            backbone_out["point_inputs_per_frame"][0] = point_inputs

            for i in range(self.num_ortho_slices):
                new_points, new_labels = get_ortho_point(prompt_points=points, 
                                                 prompt_type=prompt_type, 
                                                 center_slice=input.center_slices, 
                                                 ortho_dim=i)
                new_point_inputs = {"point_coords": new_points, "point_labels": new_labels}
                backbone_out["point_inputs_per_frame"][i+1] = new_point_inputs
    ...
    #             print(point_inputs["point_coords"])
    #             print(t)
    #             assert 1 == 2
